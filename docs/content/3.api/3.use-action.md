---
title: useAction
description: API reference for the useAction Vue composable — reactive execution, status tracking, validation errors, and lifecycle callbacks for type-safe server actions.
---
...
|--------|------|---------|-------------|
| `detached` | `boolean` | `false` | Use the browser's native `fetch()` instead of Nuxt's `$fetch`. Native fetch is not tied to the component lifecycle, so requests survive SPA navigation and component unmounting. Useful for auto-save and other background operations. |
...
### Detached Mode (Navigation-Safe)

```vue
<script setup lang="ts">
import { saveDocument } from '#safe-action/actions'

// With detached: true, the save request keeps running
// even if the user navigates away from this page.
const { executeAsync } = useAction(saveDocument, {
  detached: true,
  onError({ error }) {
...
async function autoSave(payload: { id: string; content: string }) {
