<template>
  <div style="max-width: 640px; margin: 2rem auto; font-family: system-ui, sans-serif">
    <h1>nuxt-safe-action example</h1>

    <!-- Greet Action -->
    <section style="margin-top: 2rem">
      <h2>Greet Action</h2>
      <form @submit.prevent="greetAction.execute({ name: greetName })">
        <input
          v-model="greetName"
      ...
      <div v-if="greetAction.validationErrors.value" style="margin-top: 1rem; color: red">
        {{ JSON.stringify(greetAction.validationErrors.value) }}
      </div>
    </section>
    ...
    <section style="margin-top: 3rem">
      ...
      >
        ...
        <pre style="margin-top: 0.5rem">{{ JSON.stringify(createPostAction.data.value, null, 2) }}</pre>
      </div>
